...
class Store
{
    ...
    private function subGet($columns, $data)
    {
        ...
        $this->getFilterArray(); //($this->request["filter"]) ?? [];

        // check if limit is numeric if not set it to 10
        $limit = isset($this->request["limit"]) && (int)$this->request["limit"] > 0 ? (int)$this->request["limit"] : 10;
        // check if sort_by is valid if not set it to id
        // $sort_by = isset($this->request["sort_by"]) && in_array($this->request["sort_by"], $columns) ? $data['table'] . "." . $this->request["sort_by"] : $this->model->getQuery()->from . '.id';
    ...
    private function checkImage($entity, $image_name)
    {
        if ($this->request->hasFile($image_name)) {
            ...
            $store_gallery = public_path('uploads/' . Utility::get_store_id() . '/' . $this->request->$image_name);
            if (file_exists($store_gallery)) {
                $entity[$image_name] = 'uploads/' . Utility::get_store_id() . '/' . $this->request->$image_name;
                $entity->save();
                return;
            } else {
