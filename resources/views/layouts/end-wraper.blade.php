<footer class="footer">
    <div class="primary-footer">
        <div class="container">
            <div class="row">
                <div class="col-lg-4 col-md-7 pe-lg-8">
                    <a class="navbar-brand logo footer-logo" href="index.html">
                        <img class="img-fluid" src="{{ asset('assets/portal/images/miracle-logo.png')}}" alt="" />
                    </a>
                    <p class="mt-4 mb-5">
                        مرحبًا بكم في شركة ميراكل، حيث يلتقي الابتكار بالتميز. نحن
                        ملتزمون بتقديم مجموعة شاملة من الخدمات المصممة لرفع مستوى
                        علامتك التجارية وزيادة إمكانات عملك. شغفنا يكمن في تحويل رؤيتك
                        إلى واقع من خلال خبرتنا في مختلف المجالات الابداعية و
                        الأستراتيجية.
                    </p>
                    <div class="subscribe-form">
                        <form id="mc-form" class="mc-form">
                            <input type="email" value="" name="EMAIL" class="email" id="mc-email"
                                placeholder="البريد الألكتروني" required="" />
                            <input class="subscribe-btn" type="submit" name="subscribe" value="اشترك الأن" />
                        </form>
                    </div>
                </div>
                <div class="col-lg-4 col-md-5 mt-6 mt-md-0">
                    <h5>تواصل معنا</h5>
                    <ul class="media-icon list-unstyled">
                        <li>
                            <i class="flaticon-gps-1"></i>
                            <h6>زور موقعنا</h6>
                            <p class="mb-0">اسوان شارع المطار امام مطعم المصري</p>
                        </li>
                        <li>
                            <i class="flaticon-email"></i>
                            <h6>ارسل لنا عبر البريد الألكتروني</h6>
                            <a href="mailto:themeht23@gmail.com">miraclecompany@gmail.com</a>
                        </li>
                        <li>
                            <i class="flaticon-mobile"></i>
                            <h6>اتصل بنا</h6>
                            <a href="tel:+201273061019">01273061019​</a>
                        </li>
                    </ul>
                </div>
                <div class="col-lg-4 col-md-8 mt-6 mt-lg-0">
                    <div class="row">
                        <div class="col-md-6 footer-menu">
                            <h5>الشركة</h5>
                            <ul class="list-unstyled w-100">
                                <li>
                                    <a href="about-us.html">عن ميراكل</a>
                                </li>
                                <li>
                                    <a href="services.html">خدماتنا</a>
                                </li>
                                <li>
                                    <a href="portfolio-masonry.html">معرض الاعمال</a>
                                </li>
                                <li>
                                    <a href="team.html">اعضاء الاعمال</a>
                                </li>
                                <li>
                                    <a href="contact.html">تواصل معنا</a>
                                </li>
                            </ul>
                        </div>
                        <div class="col-md-6 mt-5 mt-md-0 footer-menu">
                            <h5>عن الشركة</h5>
                            <ul class="list-unstyled">
                                <li>
                                    <a href="blog-single.html">المدونة</a>
                                </li>

                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="secondary-footer">
        <div class="container">
            <div class="copyright">
                <div class="row align-items-center">
                    <div class="col-md-6">
                        حقوق النشر 2024
                        <u>
                            <a href="#">لميراكل</a>
                        </u>
                        |جميع الحقوق محفوظة
                    </div>
                    <div class="col-md-6 text-md-end mt-3 mt-md-0">
                        <ul class="list-inline ps-0 ms-0 footer-social">
                            <li class="list-inline-item">
                                <a href="#">
                                    <i class="flaticon-facebook-app-symbol"></i>
                                </a>
                            </li>
                            <li class="list-inline-item">
                                <a href="#">
                                    <i class="flaticon-dribble"></i>
                                </a>
                            </li>
                            <li class="list-inline-item">
                                <a href="#">
                                    <i class="flaticon-instagram"></i>
                                </a>
                            </li>
                            <li class="list-inline-item">
                                <a href="#">
                                    <i class="flaticon-twitter-2"></i>
                                </a>
                            </li>
                            <li class="list-inline-item">
                                <a href="#">
                                    <i class="flaticon-linkedin-1"></i>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="dark-bg ht-bg-move"></div>
</footer>

<!--footer end-->
</div>

<div class="color-customizer closed">
    <a class="opener" href="#"> <i class="bi bi-palette"></i></a>
    <div class="clearfix color-chooser text-center">
        <h4 class="mb-4">Seoland With Awesome Colors</h4>
        <ul class="colorChange clearfix">
            <li class="theme-default selected" title="theme-default" data-style="color-1"></li>
            <li class="theme-2" title="theme-2" data-style="color-2"></li>
            <li class="theme-3" title="theme-3" data-style="color-3"></li>
            <li class="theme-4" title="theme-4" data-style="color-4"></li>
            <li class="theme-5" title="theme-5" data-style="color-5"></li>
            <li class="theme-6" title="theme-6" data-style="color-6"></li>
            <li class="theme-7" title="theme-7" data-style="color-7"></li>
            <li class="theme-8" title="theme-8" data-style="color-8"></li>
            <li class="theme-9" title="theme-9" data-style="color-9"></li>
        </ul>
    </div>
</div>

<!--color-customizer end-->

<!--back-to-top start-->

<div class="scroll-top">
    <svg class="scroll-circle svg-content" width="100%" height="100%" viewBox="-1 -1 102 102">
        <path d="M50,1 a49,49 0 0,1 0,98 a49,49 0 0,1 0,-98" />
    </svg>
</div>

<!--back-to-top end-->

<!-- inject js start -->

<!--== jquery -->
<script src="{{ asset('assets/portal/js/jquery.min.js')}}"></script>

<!--== bootstrap -->
<script src="{{ asset('assets/portal/js/bootstrap.bundle.min.js')}}"></script>

<!--== jquery-appear -->
<script src="{{ asset('assets/portal/js/jquery-appear.js')}}"></script>

<!--== magnific-popup -->
<script src="{{ asset('assets/portal/js/jquery.magnific-popup.min.js')}}"></script>

<!--== isotope -->
<script src="{{ asset('assets/portal/js/isotope.pkgd.min.js')}}"></script>

<!--== counter -->
<script src="{{ asset('assets/portal/js/odometer.min.js')}}"></script>

<!--== countdown -->
<script src="{{ asset('assets/portal/js/jquery.countdown.min.js')}}"></script>

<!--== gsap -->
<script src="{{ asset('assets/portal/js/gsap.min.js')}}"></script>

<!--== scrolltrigger -->
<script src="{{ asset('assets/portal/js/scrolltrigger.min.js')}}"></script>

<!--== gsap-animation -->
<script src="{{ asset('assets/portal/js/gsap-animation.js')}}"></script>

<!--== particles -->
<script src="{{ asset('assets/portal/js/particles.js')}}"></script>

<!--== swiper -->
<script src="{{ asset('assets/portal/js/swiper-bundle.min.js')}}"></script>

<!--== swiper-init -->
<script src="{{ asset('assets/portal/js/seoland-swiper-init.js')}}"></script>

<!--== sticksy -->
<script src="{{ asset('assets/portal/js/sticksy.min.js')}}"></script>

<!--== color-customize -->
<script src="{{ asset('assets/portal/js/color-customize/color-customizer.js')}}"></script>

<!--== theme-script -->
<script src="{{ asset('assets/portal/js/theme-script.js')}}"></script>
